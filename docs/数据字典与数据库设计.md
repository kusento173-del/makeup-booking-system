...
> **当前数据模型修订：** 五类角色均使用密码凭据；`account_binding_codes` 和 `auth_binding_challenges` 已由迁移 `20260727123500_remove_wechat_binding_tables` 删除；客户端类型只使用 `MOBILE_WEB`、`ADMIN_WEB` 及内部任务类型。本文后续旧绑定表设计仅作历史背景，不代表当前 Schema，执行事实以 Prisma Schema 和迁移为准。
