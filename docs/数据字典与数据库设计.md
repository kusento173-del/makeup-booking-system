...
> **当前数据模型修订：** 五类角色均使用密码凭据；`account_binding_codes` 和 `auth_binding_challenges` 已由迁移 `20260727123500_remove_wechat_binding_tables` 删除；迁移 `20260727210000_revoke_legacy_wechat_identities` 已撤销历史微信身份，并禁止任何非密码身份再次处于有效状态；客户端类型只使用 `MOBILE_WEB`、`ADMIN_WEB` 及内部任务类型。本文后续旧绑定表设计仅作历史背景，不代表当前 Schema，执行事实以 Prisma Schema 和迁移为准。
...
- PostgreSQL 是人员、预约、固定关系、班次、审批和后台任务的唯一事实来源。
...
- 未删除人员统一为正常；主播另有正常、取消资格两种预约资格。
- 预约只使用已预约、已取消、已完成。
- 审批申请使用待审核、已通过、已驳回、已撤回。
- 主播、化妆师和运营使用 `deleted_at` 实施保留历史的业务删除；不得物理删除其预约、固定规则、关系或审计记录。
- 其他核心业务表仍使用有明确含义的状态或有效期，不实施通用级联软删除。
...
- 只有纯从属技术数据可以级联删除，例如尚未确认导入批次的暂存行。
...
| `employment_status` | `ACTIVE`、`INACTIVE` | 化妆师、运营内部兼容状态；业务界面不可直接修改 |
| `host_qualification_status` | `ACTIVE`、`CANCELLED` | 主播预约资格 |
...
| `status` | `varchar(16)` | 是 | `PENDING_BINDING`、`ACTIVE`、`DISABLED` |
...
| `password_hash` | `text` | 是 | 使用 Argon2id 强密码哈希，绝不保存明文 |
...
客服和管理员后台账号使用本表；微信登录人员不需要创建密码凭据。连续失败 5 次后通过 `locked_until` 锁定 15 分钟；成功登录或改密后清零失败次数。改密会撤销该账号全部有效会话。
...
| `site_id` | `uuid` | 按角色 | 主播、运营、化妆师和客服必须有场地；仅管理员为空代表全局 |
| `assigned_by_user_id` | `uuid` | 否 | 分配人 |
| `assigned_at` | `timestamptz` | 是 | 生效时间 |
| `revoked_at` | `timestamptz` | 否 | 撤销时间 |

同一账号可以有多个角色。有效角色为 `revoked_at IS NULL` 的记录。微信绑定主播、运营或化妆师时，角色场地必须同步取自对应人员档案，后续会话和业务权限均以该角色场地进行隔离。
...
| `id` | `uuid` | 是 | 会话 ID |
| `user_id` | `uuid` | 是 | 外键至 `app_users` |
| `role_assignment_id` | `uuid` | 有效会话是 | 外键至本账号的 `user_roles`；旧会话升级时撤销并允许留空 |
...
会话记录本次登录所选的角色，但不把它当作永久权限缓存。每次签发访问令牌时必须从有效 `user_roles` 重新校验当前角色；业务入口还要同时校验账号、会话和角色均未撤销。访问令牌有效期 15 分钟，刷新令牌有效期 30 天且每次使用后立即轮换，数据库只保存 SHA-256 摘要。退出登录只撤销会话，不删除历史记录。

#### 6.1.7 `account_binding_codes` 一次性账号绑定码

| 字段 | 类型 | 必填 | 约束/说明 |
|---|---|---:|---|
| `id` | `uuid` | 是 | 绑定码记录 ID |
| `role_code` | `varchar(32)` | 是 | 仅允许主播、运营、化妆师 |
| `site_id` | `uuid` | 是 | 发码和权限校验场地 |
| `host_profile_id` / `artist_profile_id` / `operator_profile_id` | `uuid` | 条件必填 | 按角色严格且仅填写一个目标档案 |
| `code_hash` | `char(64)` | 是 | 唯一，只保存带服务端密钥的摘要 |
| `expires_at` | `timestamptz` | 是 | 默认签发后 24 小时 |
| `failed_attempt_count` / `max_attempts` | `integer` | 是 | 默认最多尝试 5 次 |
| `consumed_at` / `consumed_by_user_id` | 时间 / UUID | 否 | 成功绑定后成对填写 |
| `revoked_at` / `revoked_by_user_id` / `revoke_reason` | 时间 / UUID / 文本 | 否 | 重新发码或人工撤销时成组填写 |
| `created_by_user_id` | `uuid` | 是 | 发码人 |
| `created_at` / `updated_at` / `row_version` | 时间 / 整数 | 是 | 审计与并发控制 |

同一人员同时只能有一个未消费、未撤销的绑定码；过期码在重新签发时先撤销。绑定码记录禁止物理删除。手机号不参与第一期账号绑定。

#### 6.1.8 `auth_binding_challenges` 短期绑定凭证

| 字段 | 类型 | 必填 | 约束/说明 |
|---|---|---:|---|
| `id` | `uuid` | 是 | 凭证记录 ID |
| `user_id` | `uuid` | 是 | 微信身份对应的待绑定账号 |
| `token_hash` | `char(64)` | 是 | 唯一，只保存 256 位随机令牌的 SHA-256 摘要 |
| `expires_at` | `timestamptz` | 是 | 签发后 10 分钟 |
| `consumed_at` / `revoked_at` | `timestamptz` | 否 | 成功绑定或重新登录时终结，不能同时存在 |
| `created_at` | `timestamptz` | 是 | 创建时间 |

微信登录码只由服务端提交给微信换取 OpenID。系统先把微信身份保存在待绑定账号，客户端只收到高熵短期绑定凭证，不能自行提交或伪造 OpenID。绑定凭证禁止物理删除。

#### 6.1.9 `auth_role_selection_challenges` 短期角色选择凭证

| 字段 | 类型 | 必填 | 约束/说明 |
|---|---|---:|---|
| `id` | `uuid` | 是 | 凭证记录 ID |
| `user_id` | `uuid` | 是 | 已由微信验证且拥有多个有效角色的账号 |
| `token_hash` | `char(64)` | 是 | 唯一，只保存 256 位随机令牌的 SHA-256 摘要 |
| `expires_at` | `timestamptz` | 是 | 签发后 5 分钟 |
| `consumed_at` / `revoked_at` | `timestamptz` | 否 | 成功选取角色或重新签发时终结，不能同时存在 |
| `created_at` | `timestamptz` | 是 | 创建时间 |

多角色账号必须持一次性角色选择凭证创建正式会话，不能仅凭客户端提交的账号 ID 和角色 ID 登录。每个账号同时只能有一个有效凭证，历史记录禁止物理删除。
...
| `qualification_valid_until` | `date` | 否 | 取消资格含截止日期；次日按正常资格处理 |
| `deleted_at` | `timestamptz` | 否 | 人员业务删除时间；非空时禁止登录和新业务 |
...
取消资格有效期内禁止新增预约；最长一个月，截止日期次日自动恢复。业务删除与资格取消是两套独立语义，均须记录审计。
...
| `deleted_at` | `timestamptz` | 否 | 人员业务删除时间 |
...
首批主播—运营对应表未提供开始日期，首次迁移时由导入服务使用批次正式确认的上海业务日期填入 `valid_from`，`valid_until` 保持 `NULL`。因此关系表始终只有明确的开始日期和可选结束日期，不引入“目标日期”字段。
...
| `status` | `varchar(16)` | 是 | `PENDING`、`ACTIVE`、`REJECTED`、`CANCELLED` |
| `reason` | `varchar(500)` | 否 | 请假原因 |
| `created_by_user_id` | `uuid` | 是 | 提交人 |
| `created_at` | `timestamptz` | 是 | 提交时间 |
| `reviewed_by_user_id` | `uuid` | 否 | 化妆师请假审核人 |
| `reviewed_at` | `timestamptz` | 否 | 审核时间 |
| `review_comment` | `varchar(500)` | 否 | 审核意见；驳回必填 |
| `review_impact_snapshot` | `jsonb` | 否 | 审批当时的受影响预约数组，保存主播、编号、预约时间和固定/单次类型 |
...
#### 6.3.5 `artist_unavailable_periods` 化妆师临时不可排班时段

| 字段 | 类型 | 必填 | 约束/说明 |
|---|---|---:|---|
| `id` | `uuid` | 是 | 主键 |
| `artist_id` | `uuid` | 是 | 化妆师 |
| `site_id` | `uuid` | 是 | 场地快照，必须与化妆师所属场地一致 |
| `unavailable_date` | `date` | 是 | 临时不可排班日期 |
| `start_minute` | `smallint` | 是 | 开始分钟，必须为 15 的倍数 |
| `end_minute` | `smallint` | 是 | 结束分钟，必须为 15 的倍数且大于开始时间 |
| `minute_range` | `int4range` | 是 | 数据库生成的半开区间 `[start, end)` |
| `reason` | `varchar(500)` | 是 | 上课或其他不可排班原因 |
| `status` | `varchar(16)` | 是 | `PENDING`、`ACTIVE`、`REJECTED`、`CANCELLED` |
| `created_by_user_id` | `uuid` | 是 | 创建人 |
| `reviewed_by_user_id` | `uuid` | 否 | 化妆师本人申请时的审核人 |
| `reviewed_at` | `timestamptz` | 否 | 审核时间 |
| `review_comment` | `varchar(500)` | 否 | 审核意见；驳回必填 |
| `review_impact_snapshot` | `jsonb` | 否 | 审批当时的受影响预约数组，审批历史只读取该快照 |
| `cancelled_by_user_id` | `uuid` | 否 | 撤销人 |
| `cancelled_at` | `timestamptz` | 否 | 撤销时间 |
| `cancellation_reason` | `varchar(500)` | 否 | 撤销原因 |
| `affected_appointment_count` | `integer` | 是 | 创建时取消的重叠预约数 |
| `row_version` | `integer` | 是 | 乐观锁版本 |

同一化妆师、同一日期的待审核或有效时段通过 GiST 排斥约束禁止重叠；相邻时段允许分别保存。未来七日、零点锁定、必须落入当日工作或批准加班区间等动态规则由服务层在事务内校验。

两个请假表的 `review_impact_snapshot` 均只能为 JSON 数组。审批事务在重新确认影响数量后写入快照，审批历史不再通过当前预约状态反推过去的影响，避免取消请假或恢复固定预约后历史明细消失。
...
第一阶段不包含通知模板、订阅授权、通知任务或发送尝试表。`outbox_events` 仅用于确有异步消费者的内部任务，目前用于排班导出；预约事务不为通知预写事件。
...
|---|---|---:|---|
| `id` | `uuid` | 是 | 事件 ID |
| `event_type` | `varchar(64)` | 是 | 当前为排班导出等内部异步任务 |
...
创建导出请求时，导出记录与外盒事件在同一事务写入；Worker 幂等生成文件。不存在消费者的业务动作不得写入外盒。
...
`operation_logs` 使用数据库触发器拒绝 `UPDATE` 和 `DELETE`，生产数据库账号仍应按最小权限禁止应用修改或删除该表。日志 JSON 在服务端递归移除敏感字段，数据库再次拒绝密码、令牌、完整手机号和微信身份字段；手机号末四位可以保留用于人工核对。
...
| `storage_deleted_at` | `timestamptz` | 否 | 过期文件实际清理时间；只允许成功任务写入一次 |
...
首期建议参数：可预约天数 7、固定生成覆盖 8 日和业务时区。每天最多 2 次属于数据库和代码共同固定的核心规则，不作为可随意修改的运行时配置。其他参数也需在代码中设置安全边界，防止错误配置。
...
- 主播或化妆师请假导致固定预约取消后，固定生成任务不得新建重复记录；截止前撤销化妆师请假时，可在原记录上恢复仍有效的固定预约，取消主播请假不自动恢复。
- 固定预约实例取消后，固定占用查询忽略该规则在对应日期的时段，使空档可被单次预约；其他日期仍按固定规则占用。
...
1. 加班申请是否设置自动失效时间；当前默认一直待审核，直至通过、驳回或撤回。
...
该模型使用内部 UUID 保证人员身份稳定，使用有效期和版本保存关系历史，使用 PostgreSQL 排斥约束保证预约并发正确，并使用离散 15 分钟格解决固定规则和待审核申请的长期档期占用。核心业务数据保持结构化，JSON 只用于导入原始值和审计快照。
