...
| `business_type` | `varchar(32)` | 是 | 预约、00:01 汇总、提前一小时等 |
...
首期建议参数：可预约天数 7、固定生成覆盖 8 日、每日汇总时间 00:01 和业务时区。每天最多 2 次属于数据库和代码共同固定的核心规则，不作为可随意修改的运行时配置。其他参数也需在代码中设置安全边界，防止错误配置。
...
- 预约日期当天 00:00 锁定后，以 00:01 查询到的当日事实生成运营和化妆师通知任务；主播不生成每日汇总。
- 运营汇总按当日有效运营关系确定人员范围，必须包含无预约主播。
- 化妆师汇总包含无预约、非工作日和加班日对应状态。
- 未绑定账号或没有可用通知授权的对象仍创建失败可见的任务记录，不静默遗漏。
- 同一接收人和业务日期只生成一次，重试不得重复创建任务。
...
8. 实现固定生成、自动完成、00:01 汇总和一小时前提醒任务。
...
1. 微信通知最终使用哪种可用消息能力，以及授权失败时是否启用短信。
2. 加班申请是否设置自动失效时间；当前默认一直待审核，直至通过、驳回或撤回。
