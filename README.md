...
当前阶段：**M8 小程序业务端**。M0 至 M7 已完成文档与工程基线、身份与主数据、班次与请假、普通预约、固定关系、排班导出及后台任务；当前后端、数据库和管理后台核心能力已经成型，小程序已完成微信登录、一次性绑定和角色选择，正在补齐三类移动角色的业务页面。
...
首次运行前，在微信开发者工具的“设置—安全设置”中开启服务端口。该命令只打开首页、排班、班次、请假和加班页面并检查是否真实渲染，不提交任何业务表单，也不会关闭开发者工具。若命令提示“页面实例存在，但没有渲染任何视图”，先执行“清除缓存—清除全部缓存”，再按 `Ctrl+B` 重新编译后重试；这通常表示开发者工具仍在使用旧编译缓存，不能把单元测试或 `dist` 文件存在当作微信端已通过验收。
...
按[当前迭代](docs/当前迭代.md)进入 `M8-MINIAPP-SHELL-01`：建立按当前登录角色展示的小程序业务壳层和导航，不重复实现服务端业务规则。
