...
| `provider` | `varchar(32)` | 是 | 例如 `WECHAT_MINIPROGRAM`、`PASSWORD` |
| `provider_app_id` | `varchar(128)` | 否 | 微信应用或登录域标识 |
...
| 运营人员表 | `operator_profiles` | 生成 UUID；先补齐关系表引用的有效运营 |
| 主播与运营对应表 | `host_operator_relations` | 解决未知引用；无运营不建关系行；批次参数提供生效日 |
| 化妆师固定主播名单 | 暂存区，补齐后写入固定相关表 | 当前只能作为候选；必须补齐星期、时间、时长和开始日期并解决跨场地问题 |
...
1. 手机端最终使用微信小程序还是微信内网页。
2. 登录绑定采用手机号验证、一次性绑定码或两者结合。
3. 微信通知最终使用哪种可用消息能力，以及授权失败时是否启用短信。
4. 当前 75 条无法匹配运营关系的最终处理结果。
5. 338 条固定候选关系的星期、开始时间、时长和开始日期来源。
6. 8 条跨场地固定候选关系的修正结果。

其中第 4～6 项不阻塞代码骨架和功能开发，但阻塞生产数据迁移。
